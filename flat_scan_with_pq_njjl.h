    ...
    for (size_t m = 0; m < M; m++) {
      ...
      for (size_t k = 0; k < K; k++) {
        ...
        if (d_sub >= 4) {
          ...
          // 处理剩余元素
          for (; j < d_sub; j++) {
            dot += x[m * d_sub + j] * codebooks[m][k * d_sub + j];
          }
        } else {
          // 标量计算
          for (size_t j = 0; j < d_sub; j++) {
    ...
    for (size_t m = 0; m < M; m++) {
      for (size_t k = 0; k < K; k++) {
        ...
        if (d_sub >= 4) {
          ...
          // 处理剩余元素
          for (; j < d_sub; j++) {
            dot += query[m * d_sub + j] * codebooks[m][k * d_sub + j];
          }
        } else {
          // 标量计算
          for (size_t j = 0; j < d_sub; j++) {
        ...
        // 存储负内积作为距离
        const_cast<float &>(distance_tables[m][k]) = -dot;
      }
    }
  }
    ...
    for (size_t j = 0; j < k; j++) {
      if (counts[j] == 0) {
        ...
        if (counts[max_count_idx] > 1) {
          ...
          // 复制中心并添加一些随机扰动
          for (size_t d_idx = 0; d_idx < d; d_idx++) {
            float noise = (rng() % 1000) / 10000.0f -
                          0.05f; // 添加 -0.05 到 0.05 之间的噪声
            centroids[j * d + d_idx] =
                centroids[max_count_idx * d + d_idx] + noise;
          }
        }
      }
    }
...
class PQIndex {
public:
  PQIndex(size_t d, size_t M = 4, size_t K = 256) : dim(d), pq(d, M, K), M(M) {}

  // 构建索引
  void build(const float *data, size_t n) {
    std::cout << "开始构建PQ索引，数据点数: " << n << ", 维度: " << dim
              << std::endl;
  ...
  void save(const std::string &filename) const {
    ...
    // 写入原始数据 (可能会很大)
    fout.write(reinterpret_cast<const char *>(original_data.data()),
  ...
  void load(const std::string &filename) {
    ...
    fin.read(reinterpret_cast<char *>(original_data.data()),
             original_data.size() * sizeof(float));
...
// 使用PQ在基础向量集上搜索查询向量的k个最近邻
std::priority_queue<std::pair<float, uint32_t>>
flat_search_with_pq(float *base, float *query, size_t base_number,
